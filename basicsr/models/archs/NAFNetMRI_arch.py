    ...
    def forward(self, adc, adc_roi, t2, t2_roi, dwi, dwi_roi):
        ...
        for k in range(adc.size(1)):
            if adc_each_channel_roi[k] > 0:
                ...
                adc_feats.append(bnc_feats)
        adc_feats = torch.cat(adc_feats, dim=1)

        # print('adc_feats.. ', adc_feats.shape)
        # print('debug adc feats .. ', adc_feats.shape)


        adc_pred = self.adc_head(adc_feats.mean(dim=1))
        ...
        for k in range(t2.size(1)):
            if t2_each_channel_roi[k] > 0:
                ...
                t2_feats.append(bnc_feats)
        t2_feats = torch.cat(t2_feats, dim=1)


        # print('debug t2 feats .. ', t2_feats.shape)

        t2_pred = self.t2_head(t2_feats.mean(dim=1))


        return 0.5 * (adc_pred[:, 0] + t2_pred[:, 0])
