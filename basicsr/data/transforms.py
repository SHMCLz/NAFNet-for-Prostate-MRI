...
def augment(imgs, hflip=True, rotation=True, flows=None, return_status=False, vflip=False):
    ...
    def _augment(img):
        if hflip:  # horizontal
            # print('debug h flip start ', img.shape, img.dtype)
            img = cv2.flip(img, 1)
            if len(img.shape) == 2:
                img = img[:, :, np.newaxis]
            # print('debug h flip finished ', img.shape, img.dtype)
            if img.shape[2] == 6:
                img = img[:,:,[3,4,5,0,1,2]] # swap left/right
        if vflip:  # vertical
            # print('debug w flip start ', img.shape, img.dtype)
            img = cv2.flip(img, 0)
            if len(img.shape) == 2:
                img = img[:, :, np.newaxis]
            # print('debug w flip finished ', img.shape, img.dtype)
        if rot90:
            # print('debug rot90 start', img.shape)
            img = img.transpose(1, 0, 2)
            # print('debug rot90 finished ', img.shape)
        return img
